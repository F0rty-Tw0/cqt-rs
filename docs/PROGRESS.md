...
| E002 | suspected | Empty live input may produce invalid JSON in the CLI's `done` event | Source computes elapsed wall seconds / zero audio seconds in `monitor/src/bin/monitor.rs`. Not yet demonstrated with an executing baseline binary. No fix claimed. |
