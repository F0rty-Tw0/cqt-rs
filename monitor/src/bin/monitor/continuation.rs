...
impl Decisions {
    ...
    fn update(
        ...
        for song in 0..self.states.len() {
            ...
            let current: Vec<_> = retrieved.iter()
                .filter(|s| usize::from(s.candidate.song) == song).copied().collect();
            ...
            for s in current.iter().filter(|s| s.alignment >= self.start_alignment) {
                if !pending.iter().any(|p| self.agrees(p.scored.candidate, s.candidate, end)) {
                    pending.push(Hypothesis {
                        scored: *s, begin, end, observations: 1,
                    });
                }
            }
            pending.sort_by(|a, b| {
                b.observations.cmp(&a.observations)
                    .then_with(|| b.scored.confidence.total_cmp(&a.scored.confidence))
                    .then_with(|| b.scored.alignment.total_cmp(&a.scored.alignment))
                    .then_with(|| a.scored.candidate.offset.total_cmp(&b.scored.candidate.offset))
            });
            if let Some(&p) = pending.iter().find(|p| {
                p.observations >= 2 && p.scored.confidence >= self.threshold
            }) {
    ...
    fn finish(&mut self) -> Vec<Change> {
        self.states.iter_mut().filter_map(|s| {
            s.pending.clear();
...
impl Continuation {
    ...
    pub(super) fn lookups(&self) -> u64 { self.matcher.lookups() }
    pub(super) fn matches(&self) -> u64 { self.matcher.matches() }
    ...
    pub(super) fn drain<W: Write>(
        ...
        loop {
            let next = ((self.observation_number + 1) as f64 * self.frames_per_observation).round() as u64;
            if next > horizon && !(eof && self.begin < horizon) { break; }
            ...
            let verify = |c: Candidate| ctx.tracks[usize::from(c.song)].verify(
                &query, c.shift, c.tempo, c.offset, ctx.verify_frames, ctx.verify_bins,
            );
            let mut scored = Vec::new();
            for c in self.matcher.hypotheses_per_song(self.decisions.budget, ctx.modal_fit) {
                let v = verify(c);
                let s = Scored { candidate: c, confidence: self.matcher.confidence(c.evidence),
            ...
            for state in &self.decisions.states {
                for (h, origin) in state.active.iter().map(|p| (p, "active"))
                    .chain(state.pending.iter().map(|p| (p, "pending"))) {
                    let c = h.scored.candidate;
                    let v = verify(c);
                    self.write_observation(ctx, Scored {
                        candidate: Candidate { evidence: 0, ..c }, confidence: 0.0,
                        alignment: v.query_fraction(),
                    }, v, origin, end, consumed, complete, out);
                }
            }
            writeln!(out,
            ...
            if complete {
                for change in self.decisions.update(self.begin, end, &scored, |c| verify(c).query_fraction()) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn write_observation<W: Write>(&self, ctx: &Context<'_>, s: Scored, v: Verification,
        origin: &str, end: u64, consumed: u64, complete: bool, out: &mut W) {
        let c = s.candidate;
        writeln!(out,
    ...
    fn write_change<W: Write>(&self, ctx: &Context<'_>, change: Change, end: u64, consumed: u64, out: &mut W) {
...
mod tests {
    ...
    fn tracker(budget: usize) -> Decisions {
        Decisions { states: vec![State::default(), State::default()], budget, threshold: 70.0,
    ...
    fn score(song: u16, evidence: u32, offset: f64) -> Scored {
        Scored { candidate: Candidate { song, evidence, shift: 0, tempo: 1.0, offset },
            confidence: 100.0 * f64::from(evidence) / (f64::from(evidence) + 40.0), alignment: 0.8 }
    }
    #[test]
    fn continuous_anchors_keep_long_context_across_partition_and_partial_tail() {
        ...
        let hashes: Vec<Hash> = [199, 200, 399, 400].into_iter().enumerate().map(|(i, frame)| Hash {
            key: encode_key(i as i32 * 20, 10, 16), frame, bin: 20, span: 320,
        }).collect();
        let peaks: Vec<_> = hashes.iter().map(|h| Peak {frame: h.frame, bin: h.bin}).collect();
        ...
        let ctx = Context { names: index.names(), tracks: &tracks, seconds_per_frame: 0.01,
            sample_rate: 100.0, delay: 344, verify_span: 200, verify_frames: 4, verify_bins: 1, modal_fit: true };
        let opts = Options {continuation_seconds: Some(2.0), window: 10.0, ..Options::default()};
    ...
    fn overlapping_retrieval_never_accumulates_confidence() {
        let mut d = tracker(3);
        for i in 0..30 {
            assert!(d.update(i*200, (i+1)*200, &[score(0, 60, 1000.0)], |_| 0.8).is_empty());
        }
        assert!(matches!(d.update(6000, 6200, &[score(0, 100, 1000.0)], |_| 0.8).as_slice(), [Change::Start(_)]));
    }
    #[test]
    fn fresh_prediction_required_before_refitting_or_confirming() {
        ...
        assert!(matches!(d.update(400, 600, &s, |_| 0.8).as_slice(), [Change::Start(_)]));
    }
    #[test]
    fn stale_high_scores_do_not_renew_or_restart_and_empty_retrieval_can_hold() {
        ...
        assert!(matches!(d.update(800, 1000, &[s], |_| 0.0).as_slice(), [Change::End(p, "release")] if p.end == 600));
    ...
    fn competing_positions_survive_until_continuation_disambiguates() {
        ...
        let mut bad = a; bad.alignment = 0.0;
        assert!(matches!(d.update(200, 400, &[bad, b], |c| if c.offset == 2000.0 { 0.8 } else { 0.0 }).as_slice(),
    ...
    fn pending_storage_is_bounded_and_trajectory_changes_need_confirmation() {
        let mut d = tracker(3);
        let many: Vec<_> = (0..20).map(|i| score(0, 1, f64::from(i)*1000.0)).collect();
        for i in 0..20 {
        ...
        assert!(d.update(400, 600, &[score(0, 100, 2000.0)], |_| 0.0).is_empty());
        let changes = d.update(600, 800, &[score(0, 100, 2000.0)], |c| if c.offset == 2000.0 {0.8} else {0.0});
        assert!(matches!(changes.as_slice(), [Change::End(_, "trajectory"), Change::Start(_)]));
    }
}
